...
function Actions_teacher() {
  ...
  const showUpdateModal = (teacher) => {
    ...
    // Debugging: Log to ensure form fields are being set correctly
    console.log('Teacher data for modal:', teacher);

    form.setFieldsValue({
      name: teacher.teacherName,
      email: teacher.email,
      cin: teacher.cin,
      isValide: teacher.isValide,
    });
  };

  const handleUpdate = async (values) => {
    // Debugging: Log the form values to verify they are correct
    ...
    const updatedValues = {
      ...
      email: values.email,
      cin: values.cin,
      isValide: values.isValide, // Make sure this value is correctly captured
    };

    // Debugging: Log the updated values to ensure correct data is sent
    console.log('Updated values to send to backend:', updatedValues);
      ...
      <section className="p-6 shadow-lg border border-gray-200 rounded-lg mr-6">
                    ...
                    {teachers.map((teacher) => (
                        <tr key={teacher.id} className="border-b dark:border-gray-700">
                          ...
                          <td className="py-5 flex items-center justify-center gap-2">
                            <Button type="primary" onClick={() => showUpdateModal(teacher)}>
                              Update
                            </Button>
                            <Button danger onClick={() => handleDelete(teacher.id)}>
                              Delete
                            </Button>
                          </td>
                        </tr>
        ...
        >
          <Form form={form} layout="vertical" onFinish={handleUpdate}>
            ...
            <Form.Item label="Is Valid" name="isValide" valuePropName="checked">
              <Switch />
            </Form.Item>
          </Form>
        </Modal>
